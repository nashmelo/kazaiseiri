...
export async function submitInquiry({
  ...
  const kintoneData = await kintoneRes.json().catch(() => null);

  if (!kintoneRes.ok) {
    console.error("submitInquiry /api/kintone error:", kintoneData);

    const message = extractErrorMessage(kintoneData);
    throw new Error(`kintone登録失敗: ${message}`);
  }

  return {
    ok: kintoneData?.ok,
    data: kintoneData?.data,
    requestId: kintoneData?.requestId || requestId,
    summaryUrl: kintoneData?.summaryUrl || "",
  };
}